...
conn = psycopg2.connect(host=settings.ocr_host, database=settings.ocr_db, user=settings.ocr_user,
                        connect_timeout=60)
...
if not os.path.exists(settings.project_id):
...
if not os.path.exists(fulltext_folder):
...
for filename in list_of_files:
    ...
    db_cursor.execute("INSERT INTO ocr_documents (project_id, filename, doc_version, doc_section, ocr_source, "
                      "doc_width, doc_height) VALUES (%(project_id)s, %(filename)s, %(doc_version)s, %(doc_section)s, "
                      "%(source)s, %(doc_width)s, %(doc_height)s) RETURNING document_id",
        {'project_id': settings.project_id, 'filename': Path(filename).name, 'doc_version': doc_version,
         'doc_section': doc_section, 'source': 'Google Vision API', 'doc_width': im.size[0], 'doc_height': im.size[1]})
    ...
    # Save word, confidence, coords to wordfile
    data_file = '{}/{}.csv'.format(csv_folder, file_stem)
    wordfile = open(data_file, "w")
    wordfile.write("word_text,block,page,word,word_line,confidence,vertices_x_0,vertices_y_0,vertices_x_1,vertices_y_1,"
        "vertices_x_2,vertices_y_2,vertices_x_3,vertices_y_3\n")
    ...
    for page in response.full_text_annotation.pages:
        for block in page.blocks:
            ...
            if block.confidence > 0.9:
                linecolor = "#66ff33"
            elif 0.9 >= block.confidence > 0.8:
                linecolor = "#ffdb4d"
            elif 0.8 >= block.confidence > 0.7:
                linecolor = "#ffa366"
            else:
                linecolor = "#ff6666"
            ...
            if settings.box_draw == "blocks":
                ...
                draw.line(
                    [(wrd_vertices_x_0_line, wrd_vertices_y_0_line), (wrd_vertices_x_1_line, wrd_vertices_y_1_line)],
                    fill=linecolor, width=settings.line_width)
                draw.line(
                    [(wrd_vertices_x_1_line, wrd_vertices_y_1_line), (wrd_vertices_x_2_line, wrd_vertices_y_2_line)],
                    fill=linecolor, width=settings.line_width)
                draw.line(
                    [(wrd_vertices_x_2_line, wrd_vertices_y_2_line), (wrd_vertices_x_3_line, wrd_vertices_y_3_line)],
                    fill=linecolor, width=settings.line_width)
                draw.line(
                    [(wrd_vertices_x_3_line, wrd_vertices_y_3_line), (wrd_vertices_x_0_line, wrd_vertices_y_0_line)],
                    fill=linecolor, width=settings.line_width)
                del draw
            db_cursor.execute("INSERT INTO ocr_blocks (document_id, block, confidence, vertices_x_0, vertices_y_0, "
                              "vertices_x_1, vertices_y_1, vertices_x_2, vertices_y_2, vertices_x_3, vertices_y_3) "
                              "VALUES (%(document_id)s,%(block)s, %(confidence)s, %(vertices_x_0)s, %(vertices_y_0)s, "
                              "%(vertices_x_1)s, %(vertices_y_1)s, %(vertices_x_2)s, %(vertices_y_2)s, "
                              "%(vertices_x_3)s, %(vertices_y_3)s)",
                {'document_id': document_id, 'block': b, 'confidence': block.confidence,
                 'vertices_x_0': wrd_vertices_x_0, 'vertices_y_0': wrd_vertices_y_0, 'vertices_x_1': wrd_vertices_x_1,
                 'vertices_y_1': wrd_vertices_y_1, 'vertices_x_2': wrd_vertices_x_2, 'vertices_y_2': wrd_vertices_y_2,
                 'vertices_x_3': wrd_vertices_x_3, 'vertices_y_3': wrd_vertices_y_3})
            for paragraph in block.paragraphs:
                ...
                for word in paragraph.words:
                    ...
                    wordfile.write("\"%s\",%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % (
                        word_text, b, p, w, word_line, word.confidence, wrd_vertices_x_0, wrd_vertices_y_0,
                        wrd_vertices_x_1, wrd_vertices_y_1, wrd_vertices_x_2, wrd_vertices_y_2, wrd_vertices_x_3,
                        wrd_vertices_y_3))
                    # write box to image
                    if word.confidence > 0.9:
                        linecolor = "#66ff33"
                    elif 0.9 >= word.confidence > 0.8:
                        linecolor = "#ffdb4d"
                    elif 0.8 >= word.confidence > 0.7:
                        linecolor = "#ffa366"
                    ...
                    if settings.box_draw == "words":
                        ...
                        if int(wrd_vertices_x_0) - settings.line_width < 0:
                            ...
                            wrd_vertices_x_0_line = int(wrd_vertices_x_0) - settings.line_width
                        draw.line([(wrd_vertices_x_0_line, wrd_vertices_y_0_line),
                                   (wrd_vertices_x_1_line, wrd_vertices_y_1_line)], fill=linecolor,
                                  width=settings.line_width)
                        draw.line([(wrd_vertices_x_1_line, wrd_vertices_y_1_line),
                                   (wrd_vertices_x_2_line, wrd_vertices_y_2_line)], fill=linecolor,
                                  width=settings.line_width)
                        draw.line([(wrd_vertices_x_2_line, wrd_vertices_y_2_line),
                                   (wrd_vertices_x_3_line, wrd_vertices_y_3_line)], fill=linecolor,
                                  width=settings.line_width)
                        draw.line([(wrd_vertices_x_3_line, wrd_vertices_y_3_line),
                                   (wrd_vertices_x_0_line, wrd_vertices_y_0_line)], fill=linecolor,
                                  width=settings.line_width)
                        del draw
                    word_list.append([word_text, word.confidence,
                                      [[wrd_vertices_x_0, wrd_vertices_y_0], [wrd_vertices_x_1, wrd_vertices_y_1],
                                       [wrd_vertices_x_2, wrd_vertices_y_2], [wrd_vertices_x_3, wrd_vertices_y_3]]])
                    db_cursor.execute("INSERT INTO ocr_entries (document_id, word_text, block, page, word, word_line, "
                                      "confidence, vertices_x_0, vertices_y_0, vertices_x_1, vertices_y_1, "
                                      "vertices_x_2, vertices_y_2, vertices_x_3, vertices_y_3) VALUES (%("
                                      "document_id)s, %(word_text)s, %(block)s, %(page)s, %(word)s, %(word_line)s, "
                                      "%(confidence)s, %(vertices_x_0)s, %(vertices_y_0)s, %(vertices_x_1)s, "
                                      "%(vertices_y_1)s, %(vertices_x_2)s, %(vertices_y_2)s, %(vertices_x_3)s, "
                                      "%(vertices_y_3)s)",
                        {'document_id': document_id, 'word_text': word_text, 'block': b, 'page': p, 'word': w,
                         'word_line': word_line, 'confidence': word.confidence, 'vertices_x_0': wrd_vertices_x_0,
                         'vertices_y_0': wrd_vertices_y_0, 'vertices_x_1': wrd_vertices_x_1,
                         'vertices_y_1': wrd_vertices_y_1, 'vertices_x_2': wrd_vertices_x_2,
                         'vertices_y_2': wrd_vertices_y_2, 'vertices_x_3': wrd_vertices_x_3,
                         'vertices_y_3': wrd_vertices_y_3})

    # If cropping is true and if there is something to crop
    if settings.crop is True and len(response.text_annotations) > 0:
        # Crop image
        results_poly = response.text_annotations[0].bounding_poly
        results_minx = min(results_poly.vertices[0].x, results_poly.vertices[1].x, results_poly.vertices[2].x,
                           results_poly.vertices[3].x)
        results_miny = min(results_poly.vertices[0].y, results_poly.vertices[1].y, results_poly.vertices[2].y,
                           results_poly.vertices[3].y)
        results_maxx = max(results_poly.vertices[0].x, results_poly.vertices[1].x, results_poly.vertices[2].x,
                           results_poly.vertices[3].x)
        results_maxy = max(results_poly.vertices[0].y, results_poly.vertices[1].y, results_poly.vertices[2].y,
                           results_poly.vertices[3].y)
